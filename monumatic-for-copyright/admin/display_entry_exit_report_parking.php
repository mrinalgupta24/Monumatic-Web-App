...
require 'connection.php';
$query = "SELECT Booking_Details.Ticket_id, Booking_Details.visitor_name,Booking_Details.Monument_Name,Booking_Details.Customer_ID,parking_entry_exit_status.Ticket_id, parking_entry_exit_status.Entry_Date,parking_entry_exit_status.Exit_Date,parking_entry_exit_status.Entry_Time,parking_entry_exit_status.Exit_Time FROM Booking_Details INNER JOIN parking_entry_exit_status ON Booking_Details.Ticket_id = parking_entry_exit_status.Ticket_id $sqladd;" ;
$query_run = mysqli_query($conn, $query);
if (mysqli_num_rows($query_run) > 0) {
foreach ($query_run as $row) {
