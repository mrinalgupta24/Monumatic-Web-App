...
if ($stmt = $conn->prepare('SELECT username, otp, time_stamp FROM `otp_authentication` WHERE username = ? and otp =?')) {
	...
	$stmt->store_result();
 $emailforfetch."<br>".$_POST['otpfromuser'];
///
if ($stmt->num_rows > 0) {
...
$timestamp = date("h:i:s");
    ...
    if ($mins < 5)
    {
        if ($sql = "UPDATE `Normal_User` SET `Password`='$passwordusr',`Date_Of_Updation`='$date' WHERE Email='$emailforfetch' and User_Id='$user_id'") {
