...
$query = "SELECT * FROM Normal_User ORDER BY Serial_Number";
	...
	if ($error === 0) {
		if ($img_size > 12500000000) {
			...
			if (in_array($img_ex_lc, $allowed_exs)) {
			    ...
			    $img_upload_path = 'userIDProof/'.$new_img_name;
...
if ($stmt = $conn->prepare('SELECT Username, password FROM Normal_User WHERE Username = ?')) {
    ...
    else 
    {
     ...
     $date =  date("Y-m-d") ;
		...
		if ($sql1 = "INSERT INTO `Normal_User`(`Serial_Number`, `User_Id`, `First_Name`, `Last_Name`, `Mobile_Number`, `Address`, `Country`, `Date_Of_Birth`, `Govt_Id_Path`, `Username`, `Email`, `Password`, `Date_Of_Creation`, `Date_Of_Updation`, `Ip_address`, `Verification_status`, `Account_Status`, `Comments`) VALUES ('$temp','$Customer_id','$firstname','$lastname','$mobilenumber','$address','$country','$dateofbirth','$img_upload_path','$username','$email','$passwordusr','$date','$date','$ip_address','$uniqid','in-active','New Account Created')")
        {
			$sql = "INSERT INTO `Normal_User`(`Serial_Number`, `User_Id`, `First_Name`, `Last_Name`, `Mobile_Number`, `Address`, `Country`, `Date_Of_Birth`, `Govt_Id_Path`, `Username`, `Email`, `Password`, `Date_Of_Creation`, `Date_Of_Updation`, `Ip_address`, `Verification_status`, `Account_Status`, `Comments`) VALUES ('$temp','$Customer_id','$firstname','$lastname','$mobilenumber','$address','$country','$dateofbirth','$img_upload_path','$username','$email','$passwordusr','$date','$date','$ip_address','$uniqid','in-active','New Account Created')";
			mysqli_query($conn, $sql);
...
try {
  ...
  // $mail->SMTPDebug=2; // Enable verbose debug output
	...
	$activate_link = 'http://localhost/monumatic/admin/user_activate.php?email=' . $_POST['email'] . '&code=' . $uniqid;
